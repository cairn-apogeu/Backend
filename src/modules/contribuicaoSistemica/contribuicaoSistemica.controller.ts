...
class ContribuicaoSistemicaController {
  ...
  async findBySprint(
    ...
    try {
      const records = await contribuicaoService.findBySprint(sprintId);
      reply.send(records);
    } catch (error) {
      reply
        .status(500)
  ...
  async findByProject(
    ...
    try {
      const records = await contribuicaoService.findByProject(projectId);
      reply.send(records);
    } catch (error) {
      reply
        .status(500)
  ...
  async findByUser(
    ...
    try {
      const records = await contribuicaoService.findByUser(userId);
      reply.send(records);
    } catch (error) {
      reply
        .status(500)
